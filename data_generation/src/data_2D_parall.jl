...
# ------------------ #
nsample = 400
nev = 30   # Number of eigenvalues/eigenvectors to compute
nt = length(tstep)
μ = 0.0    # Mean of noise
σ = 1.0    # Std of noise
dist = Normal(μ, σ)
...
# Distributed Setup      #
# ---------------------- #
# Add workers (adjust the number as needed)
if nprocs() == 1
    addprocs(5)
end
println("num procs: ", nprocs())

# Ensure necessary packages and functions are loaded on all workers
...
@everywhere begin
    ...
    function compute_pullback(Fp, col_U)
        try
            return Fp(col_U)[1]
        catch e
    ...
    function compute_gradient_with_pullback(j, cur_state_sat, pb, σ, dist)
...
@everywhere function process_sample(i, K_all, n, d, ϕ, h, tstep, q, dist, σ, nev)
    ...
    state0 = state00.state  # This is a Dict with state info
    ...
    for time_step in 1:5
        ...
        # Here we assume that the saturation information is stored in a field called :Saturations
        ...
        # Optionally, save a saturation figure
        figure()
        ...
        push!(states, cur_state)  # Store current state

        # ------------ #
        # Compute FIM  #
        # ------------ #
        # Compute pullback function
        ...
        # Compute SVD of the Jacobian-like matrix
        ...
        colorbar(fraction=0.04)
        title("Largest Left Singular Vector sample $(i) time step=$(time_step)")
    ...
    # Save outputs for this sample (define save_object as needed)
