...
def compute_fisher_svd(fvp_fn, x0, sketch_size=200, rank=50, n_iter=2):
    n = x0.numel()
    device = x0.device
    Omega = torch.randn(n, sketch_size, device=device)  # sketch_size ≫ rank
    Y = []

    for i in range(sketch_size):
        v = Omega[:, i].reshape_as(x0)
        Fv = fvp_fn(v)
        Y.append(Fv.reshape(-1))

    Y = torch.stack(Y, dim=1)  # shape: [n, sketch_size]
...
def least_squares_posterior_estimation_fisher(model, input_data, true_data, learning_rate,
    ...
    for iteration in range(num_iterations):
        ...
        U, S = compute_fisher_svd(fisher_vec_prod, x0, sketch_size=500, rank=100, n_iter=2)
        g_proj = torch.matmul(U.T, g)
        scaled = g_proj / (S**2 + 1e-6)  # Regularize inverse
        natural_grad = torch.matmul(U, scaled).reshape_as(x0)
...
if __name__ == "__main__":
    ...
    if initial_guess == "prior_mean":
        ...
        learning_rate = 0.05 #0.0001 # 0.0001 (grf, fullobs) #0.005 (noisy, fullobs) #0.00005  # Inversion learning rate.
        num_sample = 3 #50
    ...
    if initial_guess == "prior_mean":
        ...
        # prior_mean = torch.zeros(1, 1, 128, 128, requires_grad=True)
