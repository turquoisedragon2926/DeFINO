...
def least_squares_posterior_estimation_fisher(model, input_data, true_data, learning_rate,
    ...
    for iteration in range(num_iterations):
        ...
        #                                             (x0,), (v,), create_graph=False)[1] # True
        #     # Scale by observation precision matrix: (1/σ²)
        #     weighted = (jvp_out / sigma2).flatten()
        #     # Now compute JT (1/σ² Jv)
        #     fisher_product = torch.autograd.grad((weighted @ jvp_out.flatten()), x0)[0] #retain_graph=True
        ...
        def fisher_vec_prod(v):
            ...
            jvp_out = torch.autograd.functional.jvp(
                lambda x: model(x).detach()[..., i, j].flatten(),
                (x0_detached,), (v_detached,), create_graph=False
            )[1]

            weighted = (jvp_out / sigma2).flatten()
            return (weighted @ jvp_out.flatten()) * v  # scalar times v
        ...
        damping_lambda = 1e-7  # or try 1e-2, 1e-1 depending on magnitude of B
        B_damped_inv = torch.linalg.inv(B + damping_lambda * torch.eye(B.shape[0], device=B.device))
        v = Q @ (B_damped_inv @ (Q.T @ g))
        ...
        if batch_num < 2 and iteration % 10 == 0:
...
if __name__ == "__main__":
    ...
    if initial_guess == "prior_mean":
        ...
        learning_rate = 0.0001 #0.0001 # 0.0001 (grf, fullobs) #0.005 (noisy, fullobs) #0.00005  # Inversion learning rate.
        num_sample = 3 #50
